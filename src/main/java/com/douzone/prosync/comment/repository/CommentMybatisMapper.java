...
public interface CommentMybatisMapper {
    // 댓글 생성
    void createComment(CommentPostDto dto,Integer taskId);

    // 댓글 목록 조회
    List<Project> findAllComments();

    // 댓글 수정
    void updateComment(CommentPatchDto dto,Integer taskId);

    // 댓글 삭제
    void deleteComment(Integer projectId,Integer taskId,Long memberId);
}
